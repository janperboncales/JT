...
import { calculateDailyRecord, formatTime, formatDate } from '../services/attendanceLogic';
      ...
      <div className="lg:col-span-1 space-y-6">
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-6 relative overflow-hidden group">
             ...
             <h3 className="text-3xl font-bold text-white tracking-tight">
                {todaySched ? `${todaySched.startTime} - ${todaySched.endTime}` : 'No Schedule'}
             </h3>
                ...
                {schedules.length > 0 ? (
                    schedules.map(s => (
                        <div key={s.id} className="px-6 py-4 flex items-center justify-between hover:bg-zinc-800/30 transition-colors">
                            ...
                            <p className="font-mono text-sm text-zinc-500 bg-zinc-950 px-3 py-1 rounded-lg border border-zinc-800">{s.startTime} - {s.endTime}</p>
                        </div>
                    ))
                ) : (
